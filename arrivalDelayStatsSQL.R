...
function(airports, database, table){
  ...
  for (i in 1:length(airports)){
    ...
    avCMD = sprintf("SELECT AVG(ArrDelay) FROM %s WHERE origin = '%s'", table, airports[i])
    avD = dbSendQuery(con, avCMD)
    AvgArrDelay = fetch(avD)
    
    #get all arrival delays for airport
    depCMD = sprintf("SELECT ArrDelay FROM %s WHERE origin = '%s'", table, airports[i])
    ...
    state[i] = sprintf("Origin Airport: %s Avg Delay: %f  SD:%f", airports[i], AvgArrDelay, airportSD)
