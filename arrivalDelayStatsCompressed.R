#gets arrival delays for flights departing for airports of interest

#read in blocks using str.split (loop?) and calculate mean & sd

arrivalDelayStatsCompressed <- function( origin = c("LAX", 'SMF', 'OAK', 'SFO'), files, B){
  #set con
  ...
  for (i in 1:length(origin)){ #loop over airports
    ...
    while(TRUE){
      ...
      originSplit = sapply(strsplit(ll, ","), '[[', 3)
      # array of origins
